...
function stm_get_plugin_tgm_link($plugin_path, $plugin_name) {
	...
	if($install) {
		$plugin['plugin_url_activate'] = '<a class="button button-primary" href="' .esc_url( $url ) . '">' . esc_html__('Install', 'smarty') . '</a>';
	} else {
		$plugin['plugin_url_activate'] = '<a class="button button-primary" href="' .esc_url( $url ) . '">' . esc_html__('Activate', 'smarty') . '</a>';
	}
